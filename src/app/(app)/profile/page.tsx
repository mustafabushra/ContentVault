...
export default function ProfilePage() {
  const { user, loading, isAnonymous, upgradeToGoogle, logout } = useAuth()
    ...
    <>
      <div className="page">
        ...
        <div style={{ background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 20px 20px', marginBottom: 8 }}>
          <div style={{ width: 80, height: 80, borderRadius: '50%', overflow: 'hidden', marginBottom: 12, boxShadow: '0 4px 16px rgba(255,122,0,0.2)' }}>
            {user.photoURL
          ...
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
            {isAnonymous ? 'غير مسجّل' : user.email}
          </div>
          <div style={{ background: isAnonymous ? 'var(--border)' : 'linear-gradient(135deg,#FF7A00,#FFD700)', color: isAnonymous ? 'var(--text-secondary)' : 'white', padding: '4px 16px', borderRadius: 20, fontSize: 12, fontWeight: 800 }}>
        ...
        {/* Banner for guests */}
        {isAnonymous && (
          <div style={{ margin: '0 16px 12px', background: 'linear-gradient(135deg,#FF7A00,#FF4500)', borderRadius: 16, padding: '18px', color: 'white' }}>
            <div style={{ fontSize: 16, fontWeight: 800, marginBottom: 6 }}>💾 احفظ بياناتك!</div>
            <div style={{ fontSize: 13, opacity: 0.9, marginBottom: 14, lineHeight: 1.5 }}>
              سجّل دخولك بـ Google عشان ما تفقد محتواك على أي جهاز
            </div>
            <button onClick={handleUpgrade} disabled={upgrading}
              style={{ background: 'white', color: 'var(--accent)', border: 'none', padding: '11px 22px', borderRadius: 12, fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
              {upgrading ? '...' : '🔗 ربط بـ Google'}
            </button>
          </div>
        )}
        ...
        <div style={{ background: 'white', display: 'flex', justifyContent: 'space-around', padding: '16px 20px', marginBottom: 8 }}>
          {[
            ...
            { num: items.filter(it => it.isFavorite).length, label: 'مفضلة' },
        ...
        <div style={{ background: 'white', borderRadius: 16, margin: '0 16px 12px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          {[
            ...
            { icon: '❓', label: 'المساعدة', right: <span style={{ color: 'var(--text-secondary)' }}>›</span> },
          ].map((item, i, arr) => (
        ...
        {!isAnonymous && (
          <div style={{ margin: '0 16px 16px', background: 'linear-gradient(135deg,#FF7A00,#FF4500)', borderRadius: 16, padding: '20px 18px', color: 'white' }}>
            ...
            <div style={{ fontSize: 13, opacity: 0.88, marginBottom: 16, lineHeight: 1.5 }}>توليد غير محدود ودعم اللهجات وتحليل متقدم</div>
        ...
        {!isAnonymous && (
          <div style={{ background: 'white', borderRadius: 16, margin: '0 16px 24px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <div onClick={logout} style={{ textAlign: 'center', padding: '16px', cursor: 'pointer', fontSize: 15, fontWeight: 700, color: '#E53935' }}>
              تسجيل الخروج
            </div>
          </div>
        )}
